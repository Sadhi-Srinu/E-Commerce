    ...
    {/* Product List */}
    {
      list.map((item,index)=> (
        <div className="grid grid-cols-[1fr_3fr_1fr] md:grid-cols-[1fr_3fr_1fr_1fr_1fr] items-center gap-2 px-2 py-1 border text-sm" key={index}>
          ...
          <p>{item.name}</p>
          <p>{item.category}</p>
          <p className="flex flex-row">{currency}{item.price}.00</p>
          <p className="text-right md:text-center cursor-pointer text-lg"><Trash2 onClick={()=>removeProduct(item._id)} className="h-5 w-5"/></p>
        </div>
      ))
    }
