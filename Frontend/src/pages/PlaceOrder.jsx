  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      switch(method) {
        ...
        //API calls for COD
        case 'cod':
          const response = await axios.post('https://forever-backend-seven.vercel.app//api/order/place', orderData, {headers:{token}})
          ...
          case 'stripe':
            const responseStripe = await axios.post('https://forever-backend-seven.vercel.app//api/order/stripe', orderData,{headers:{token}})
  ...
  return <>
    <form onSubmit={onSubmitHandler} className="flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t">
      ...
      <div className="flex flex-col w-full sm:max-w-[480px]">
        ...
        <input required onChange={onChangeHandler} name="email" value={formData.email} type="email" placeholder="Email Address" className="border border-gray-300 rounded py-1.5 px-3.5 w-full mt-3" />
        <input required onChange={onChangeHandler} name="street" value={formData.street}  type="text" placeholder="Street" className="border border-gray-300 rounded py-1.5 px-3.5 w-full mt-3" />
        ...
        <input required onChange={onChangeHandler} name="phone" value={formData.phone}  type="number" placeholder="Phone" className="border border-gray-300 rounded py-1.5 px-3.5 w-full mt-3" />
      </div>
      ...
      <div className="mt-8">
        ...
        <div className="mt-12">
          ...
          <div className="w-full text-end mt-8">
            <button type="submit" className="text-white bg-black px-16 py-3 text-sm" >Place Order</button>
          </div>
        </div>
      </div>
    </form>
